...
function findEmployeeById(employeeId) {
 ...
 function byspecialization() {
    const byspec = employees.filter(employee => employee.specialization === 'Java');
    const byspec2 = byspec.map((employee, index) => `<p>${employee.specialization}: ${employee.id}: ${employee.name}: ${employee.department} - $${employee.salary}</p>`).join('');
    document.getElementById('employeesDetails').innerHTML = byspec2;
}